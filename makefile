...
QEMU_FLAGS    := -drive file=$(DISK_IMG),format=raw,index=0,if=ide -drive file=$(FLPY_IMG),format=raw,index=0,if=floppy
...
clean:
	rm -f $(DISK_IMG) $(FLPY_IMG) test1.tmp test2.tmp bigfile.tmp
