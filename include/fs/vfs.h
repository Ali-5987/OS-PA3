...
struct _vnode_ops {
	...
	/* the first 4 are regular file operations, from which we can read write
		data etc. Vnodes specifically support the  */

	//! open the vnode (file/device), return an opened vnode object
	vnode* 		(*open)  (vnode* node, char* path, uint32_t flags);
	...
	/* some directory related functions, only readdir is supported for now, 
		but support for the rest will be added in PA4, only required there. */
...
int32_t 	vfs_unmount (char* mount_path);
