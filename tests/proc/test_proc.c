...
//-----------------------------------------------------------------------------
// TEST 12: Scheduler Initialization
//-----------------------------------------------------------------------------
void test_scheduler_init_idempotent(void)
{
    // Calling scheduler_init multiple times should be safe
    // (or at least not crash)
    scheduler_init();
    
    // Should still have valid current thread/process
    thread_t* current_thread = get_current_thread();
    process_t* current_proc = get_current_proc();
    
    ASSERT_NOT_NULL(current_thread, "no current thread after init");
    ASSERT_NOT_NULL(current_proc, "no current process after init");

    send_msg("PASSED: test_scheduler_init_idempotent");
}
